...
typedef enum {
	OP_COW_SNAPSHOT_SPLITSNAPSHOT,
	OP_COW_SNAPSHOT_MERGE,
	OP_THIN_VOLUME_MERGE,
	OP_THIN_POOL_SPLITCACHE,
	OP_THIN_POOL_UNCACHE,
	OP_THIN_POOL_REPAIR,
	OP_THIN_POOL_CACHE,
	OP_THIN_POOL_SWAPMETADATA,
	OP_CACHE_VOLUME_SPLITCACHE,
	OP_CACHE_VOLUME_UNCACHE,
	OP_CACHE_VOLUME_SPLITMIRRORS,
	OP_CACHE_VOLUME_THIN_POOL,
	OP_CACHE_POOL_SPLITCACHE,
	OP_CACHE_POOL_SWAPMETADATA,
	OP_MIRROR_NUMBER,
	OP_MIRROR_SPLITMIRRORS,
	OP_MIRROR_LOG,
	OP_MIRROR_REPAIR,
	OP_MIRROR_LINEAR,
	OP_MIRROR_RAID,
	OP_RAID_NUMBER,
	OP_RAID_SPLITMIRRORS,
	OP_RAID_MERGE,
	OP_RAID_REPAIR,
	OP_RAID_REPLACE,
	OP_RAID_SNAPSHOT,
	...
	OP_STRIPED_THIN,
	OP_STRIPED_CACHE,
	OP_STRIPED_THIN_POOL,
	OP_STRIPED_CACHE_POOL,
	OP_STRIPED_MIRROR,
	OP_STRIPED_RAID
} lvconvert_op_t;

struct lvconvert_params {
	lvconvert_op_t op;      /* enum defining the exact lvconvert operation */
	...
	int thin;
	int uncache;
	const char *type_str_orig; /* only arg_str_value(type_ARG), showing command syntax used */
			...
			struct lvconvert_params *lp)
{
	...
	/* Save the original command syntax that was used. */
	lp->type_str_orig = arg_str_value(cmd, type_ARG, "");

	/* This value will be set by inference. */
...
static int _convert_cow_snapshot_splitsnapshot(struct cmd_context *cmd, struct logical_volume *lv,
					       struct lvconvert_params *lp)
{
	lp->op = OP_COW_SNAPSHOT_SPLITSNAPSHOT;
...
static int _convert_thin_pool_swapmetadata(struct cmd_context *cmd, struct logical_volume *lv,
					   struct lvconvert_params *lp)
{
	lp->op = OP_THIN_POOL_SWAPMETADATA;
...
static int _convert_cache_volume_splitcache(struct cmd_context *cmd, struct logical_volume *lv,
					    struct lvconvert_params *lp)
{
	lp->op = OP_CACHE_VOLUME_SPLITCACHE;
					     ...
					     struct lvconvert_params *lp)
{
	...
	lp->op = OP_CACHE_VOLUME_SPLITMIRRORS;
...
static int _convert_cache_volume_thin_pool(struct cmd_context *cmd, struct logical_volume *lv,
					   struct lvconvert_params *lp)
{
	lp->op = OP_CACHE_VOLUME_THIN_POOL;
	return _lvconvert_pool(cmd, lv, lp);
}
					  ...
					  struct lvconvert_params *lp)
{
	...
	lp->op = OP_CACHE_POOL_SPLITCACHE;
...
static int _convert_cache_pool_swapmetadata(struct cmd_context *cmd, struct logical_volume *lv,
					    struct lvconvert_params *lp)
{
	lp->op = OP_CACHE_POOL_SWAPMETADATA;
	return _lvconvert_pool(cmd, lv, lp);
}
			      ...
			      struct lvconvert_params *lp)
{
	...
	/* !strcmp(lp->type_str_orig, SEG_TYPE_NAME_CACHE) || arg_is_set(cmd, cache_ARG) */
	if (lp->cache)
				 ...
				 struct lvconvert_params *lp)
{
	...
	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_THIN_POOL) || arg_is_set(cmd, thinpool_ARG))
			   ...
			   struct lvconvert_params *lp)
{
	...
	if (strlen(lp->type_str_orig))
		new_segtype = get_segtype_from_string(cmd, lp->type_str_orig);
	...
	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_LINEAR))
			 ...
			 struct lvconvert_params *lp)
{
	...
	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_SNAPSHOT) || arg_is_set(cmd, snapshot_ARG))
		return _convert_raid_snapshot(cmd, lv, lp);

	/* !strcmp(lp->type_str_orig, SEG_TYPE_NAME_THIN) || arg_is_set(cmd, thin_ARG) */
	if (lp->thin)
	...
	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_CACHE_POOL) || arg_is_set(cmd, cachepool_ARG))
		return _convert_raid_cache_pool(cmd, lv, lp);

	if (new_segtype && segtype_is_raid(new_segtype))
		return _convert_raid_raid(cmd, lv, lp);

	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_STRIPED) || (arg_is_set(cmd, stripes_long_ARG) || arg_is_set(cmd, stripes_ARG)))
			    ...
			    struct lvconvert_params *lp)
{
	...
	if (!strcmp(lp->type_str_orig, SEG_TYPE_NAME_MIRROR))
 ...
 *
 * These functions have to purposes:
 *
 * 1. They define each possible lvconvert operation.
 * 2. They define the exact command line syntax that uniquely
 *    identifies each of those operations.
 *
 */
		      ...
		      struct lvconvert_params *lp)
{
	...
	if (strcmp(lp->type_str, lp->type_str_orig))
		log_debug("type_str %s type_str_orig %s", lp->type_str, lp->type_str_orig);
